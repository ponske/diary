...
- 移動時間と時刻計算:
  ...
  - 各アトラクション:
    ...
    - 次の `currentTime` = 出発時刻。
- 疲労度グラフ
 - HPの増減が可視化できるグラフが欲しい(競合との差分機能になるので強化)
...
- 機能:
  ...
  - スタート地点を目立たせる
  - 初回読み込み時は、ユーザーがたどる順に、ルートを表す線が伸びていく様を動的に表示させる（シミュレータとして楽しむ場合、この機能が一番ユーザー体験満足度が上がるので重要）
...
- マーカーの見た目:
  ...
  - ピンタップ時に
    ...
    - 名称
    - 優先度
  をポップアップで表示。ポップアップ表示はタップで表示/非表示が切り替えられるように
...
- Instagramなどでこのルートで回ったよという投稿ができるようにする
---
### 5. 非機能仕様（データ・ストレージ）
...
5. すべて訪問するまで3–4を繰り返す。
6. 総地点数が10以下の場合、全探索で10!通りのルートを計算し、最小となる総所要時間をlen関数で見つけ、ユーザーに返す。
