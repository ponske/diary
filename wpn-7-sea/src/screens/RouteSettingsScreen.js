...
export default function RouteSettingsScreen({ route, navigation }) {
  ...
  const handleOptimize = async () => {
    try {
      ...
      if (!validateTimeHHMM(startTime)) {
        Alert.alert('エラー', '開始時刻が無効です（例: 08:00）');
        return;
      }
      const clampedStart = clampTime(startTime, 8 * 60, 21 * 60);
      ...
      if (endTimeOption === 'custom') {
        ...
        const clampedEnd = clampTime(customEndTime, 8 * 60, 21 * 60);
      ...
      // 全探索は「可変訪問地点（アトラクション）」10件まで
      if (optimizationMethod === 'exhaustive') {
        const variableCount = selectedAttractions.filter((x) => x?.type === 'attraction').length;
        if (variableCount > 10) {
          Alert.alert('全探索は実行できません', '全探索は可変訪問地点が10地点以下の場合のみ利用できます。地点数を減らしてください。');
          return;
        }
      }
      ...
      await StorageService.setWalkingSpeed(Math.round(Number(walkingSpeed)) || 80);
      ...
      const optimizer = new RouteOptimizer(waitingTimes, Math.round(Number(walkingSpeed)) || 80);
  ...
  return (
    <AppBackground>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: Math.max(16, insets.top + 16) }]}>
        ...
        <GlassPanel style={styles.section}>
          ...
          <Text style={styles.hint}>範囲: 08:00 - 21:00（範囲外は自動補正）</Text>
          {showStartPicker && (
        ...
        <GlassPanel style={styles.section}>
          ...
          <Text style={styles.speedValue}>{Math.round(Number(walkingSpeed) || 80)} m/分</Text>
          <Slider
            minimumValue={40}
            maximumValue={140}
            step={5}
            value={Number(walkingSpeed) || 80}
            minimumTrackTintColor={Theme.colors.primary}
            maximumTrackTintColor={Theme.colors.strokeSoft}
            thumbTintColor={Theme.colors.dark}
            onValueChange={(v) => setWalkingSpeed(Math.round(v))}
          />
