...
export default function SettingsScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      setMapSource(await StorageService.getMapSource());
    })();
  }, []);
  ...
  const save = async () => {
    ...
    await StorageService.setMapSource(mapSource || 'apple');
    navigation.goBack();
  };
  ...
  return (
    <AppBackground>
      <ScrollView contentContainerStyle={[styles.content, { paddingTop: Math.max(16, insets.top + 16) }]}>
        ...
        <GlassPanel style={styles.section}>
          <Text style={styles.sectionTitle}>地図表示</Text>
          {[
            { key: 'apple', label: 'Apple マップ' },
            { key: 'osm', label: 'OpenStreetMap（OSM）' },
          ].map((x) => (
            <TouchableOpacity
              key={x.key}
              style={[styles.methodButton, mapSource === x.key && styles.methodButtonActive]}
              onPress={() => setMapSource(x.key)}
            >
              <Text style={[styles.methodText, mapSource === x.key && styles.methodTextActive]}>{x.label}</Text>
            </TouchableOpacity>
          ))}
          <Text style={styles.hint}>※OSMはオンラインでタイル取得します</Text>
        </GlassPanel>
